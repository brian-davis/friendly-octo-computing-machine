module Citation
  class Base
    def alpha_producer_names(work, role = :authors)
      authors = work.send(role).select_names.to_a
    ...
    def producer_names(work, role = :authors)
      work.send(role).select_names.map { |a| a.custom_name.presence || [a.given_name, a.middle_name, a.family_name].map(&:presence).compact.join(" ") }
    end

    def producer_last_names(work, role = :authors)
      work.send(role).map { |a| a.custom_name.presence || a.family_name }.to_sentence
    end
